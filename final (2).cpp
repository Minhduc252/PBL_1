...
long long CreateID(Node* current_ref,Node* head_ref, int facultyID) {
    ...
    long long currentyear = (1900 + local_time->tm_year) % 100;
    long long studentAge = currentyear - (current_ref->Info.Age-18);
    long long nextID=1;
    long long ID= facultyID *1000000 +studentAge * 10000 + nextID;
    while (temp != current_ref) {
        if (temp->Info.Faculty.ID == facultyID && temp->Info.Age == current_ref->Info.Age && ID!= temp->Info.ID)
            nextID = ((temp->Info.ID) % 10000) + 1;
        temp = temp->next;
    }
    ID= facultyID *1000000 +studentAge * 10000 + nextID;
    return ID;
}
...
void fileImport(){
    ...
    } while (choose > count+1 || choose < 1 || isNumber(j)!=1);
    free(j);
    if(choose==count+1) 
    {
        bool check;
